...
class Resource {
  ...
  createPreloadScriptTag(url) {
    return `<link rel="preload" as="script" href="${url}" />`;
  }

  createPreloadStyleTag(url) {
  ...
  inject(html, name, locals, options) {
    ...
    if (deps) {
      ...
      if (this.config.injectCss && (options.injectCss === undefined || options.injectCss)) {
        deps.css.forEach(url => {
          if (this.config.preload) {
            preloadInject.push(this.createPreloadStyleTag(url));
          }
      ...
      if (this.config.injectJs) {
        deps.js.forEach(url => {
          if (this.config.preload) {
            preloadInject.push(this.createPreloadScriptTag(url));
          }
      ...
      html = html.replace(this.config.injectPreloadRegex, match => {
        return match + preloadInject.join('');
      });
